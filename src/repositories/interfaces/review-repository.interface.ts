...
export interface IReviewRepository extends BaseRepository<IReviewDocument> {
  ...
  findByContractIdAndReviewerId(contractId: string, reviewerId: string): Promise<IReviewDocument | null>;
  findReviewsByFreelancerId(freelancerId: string, page: number, limit: number): Promise<{ reviews: IReviewDocument[]; total: number }>;
