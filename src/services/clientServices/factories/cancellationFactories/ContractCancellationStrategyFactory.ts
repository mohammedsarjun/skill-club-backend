...
export class ContractCancellationStrategyFactory {
  ...
  constructor(@injectAll('IContractCancellationStrategy') strategies: IContractCancellationStrategy[]) {
  ...
  getStrategy(paymentType: string): IContractCancellationStrategy {
    ...
    if (!strategy) {
      throw new AppError('Unsupported payment type for contract cancellation', HttpStatus.BAD_REQUEST);
    }
    return strategy;
  }
}
