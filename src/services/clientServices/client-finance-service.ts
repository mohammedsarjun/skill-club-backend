...
import { ClientFinanceDTO, ClientTransactionDTO } from '../../dto/clientDTO/client-finance.dto';
...
import {
  mapTransactionToDTO,
  mapFinanceStatsToDTO,
} from '../../mapper/clientMapper/client-finance.mapper';
...
export class ClientFinanceService implements IClientFinanceService {
  ...
  async getFinanceData(clientId: string): Promise<ClientFinanceDTO> {
    const spentTransactionsData =
      await this._contractTransactionRepository.findSpentTransactionsByClientId(clientId);
    const refundTransactionsData =
      await this._contractTransactionRepository.findRefundTransactionsByClientId(clientId);
    ...
    const spentTransactions = spentTransactionsData.map((transaction) => {
      ...
      const freelancerName =
        transaction.purpose === 'commission'
          ? 'Platform'
    ...
    const totalFunded =
      await this._contractTransactionRepository.getTotalFundedByClientId(clientId);
    const totalRefunded =
      await this._contractTransactionRepository.findTotalRefundByClientId(clientId);
    const totalWithdrawn =
      await this._contractTransactionRepository.findTotalWithdrawalByClientId(clientId);
  ...
  async getWithdrawalHistory(
    ...
    const items = await this._contractTransactionRepository.findWithdrawalsByClientIdWithPagination(
      clientId,
      page,
      limit,
    );
