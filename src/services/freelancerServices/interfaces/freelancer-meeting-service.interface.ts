...
export interface IFreelancerMeetingService {
  ...
  getContractMeetings(freelancerId: string, contractId: string): Promise<FreelancerMeetingListItemDTO[]>;
  ...
  declineReschedule(freelancerId: string, data: { meetingId: string; reason: string }): Promise<void>;
  counterReschedule(freelancerId: string, data: { meetingId: string; proposedTime: string }): Promise<void>;
  proposeMeeting(
    freelancerId: string,
    contractId: string,
    data: FreelancerMeetingProposalRequestDTO,
  ): Promise<FreelancerMeetingProposalResponseDTO>;
  joinMeeting(freelancerId: string, meetingId: string): Promise<{ channelName: string; token: string; appId: string; uid: string }>;
}
